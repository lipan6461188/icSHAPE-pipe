...
Thermodynamics::Thermodynamics(const Thermodynamics& copyThermo) {
	isrna=copyThermo.isrna; //store the backbone type (this is superceded by alphabetName, but some code may still use it.)
	enthalpy=copyThermo.enthalpy; //set the enthalpy parameters 
	data=copyThermo.data;
	nominal_temperature=copyThermo.GetTemperature();	//set the folding temperature
	nominal_alphabetName=copyThermo.GetAlphabetName(); // the name of the alphabet opened by a call to opendat  (This is stored in case the datatable needs to be reopened after a call to SetTemperature)
	skipThermoTables=copyThermo.skipThermoTables;
...
// IMPORTANT: Thermodynamics::CopyThermodynamic was removed. Use the Thermodyanamic(Thermodynamic* copy) constructor instead.
...
// 		currentAlphabetName = data->alphabetName;
// 	}
// 	return;
// }
