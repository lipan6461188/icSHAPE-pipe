...
RNA::RNA(const char filepathOrSequence[], const RNAInputType type, 
        const char* const alphabetName, const bool allowUnknownBases, 
        const bool skipThermoTables) 
...
//read nearest neighbor parameters at desired temperature
    const char *path = getDataPath(NULL);
    if (!path)
...
int RNA::WriteCt(const char filename[], bool append, const CTCommentProvider &commentProvider) const {
    if (ct->GetNumberofStructures()>0)
...
double RNA::GetEnsembleDefect(int structurenumber) {
	...
	BreakPseudoknot(false, structurenumber);//break the pseudoknot if there are any. The partition function doesn't support pseudoknots
...
const char* RNA::GetSequence() {
    if (ct->GetSequenceLength()==0)
        return "";
    return ct->nucs + sizeof(*ct->nucs); // add one position, so the result is 0-based
}
