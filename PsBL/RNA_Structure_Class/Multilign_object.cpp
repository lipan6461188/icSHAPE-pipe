...
//default constructor
Multilign_object::Multilign_object():dsvFiles(NULL), aliFiles(NULL), progress(NULL), instance(NULL), maxPairs(0), maxDsv(0), iterations(0), SHAPESlope(0), SHAPEIntercept(0),thermo(false) {
}
...
Multilign_object::Multilign_object(const vector<vector<string> > &inputlist, const bool isrna, ProgressHandler *Progress): 
	...
	instance(NULL), thermo(isrna,isrna?DT_RNA:DT_DNA),
	maxDsv(1), iterations(2), SHAPESlope(1.8), SHAPEIntercept(-0.6), ErrorCode(0) {
...
Multilign_object::Multilign_object(const bool Multifind ,const string &outputmultifind, const vector<string> &ctfiles, ProgressHandler *Progress, const bool isrna):  
	...
	instance(NULL), thermo(isrna,isrna?DT_RNA:DT_DNA), 
	maxDsv(1), iterations(2), SHAPESlope(2.6), SHAPEIntercept(-0.8), ErrorCode(0),output_multifind(outputmultifind),ct_files(ctfiles) {
		thermo.ReadThermodynamic();
		maxPairs = AverageLength();
}
//Destructor
...
string Multilign_object::GetErrorMessage(const int error) const {
    if (error==0) return "No Error.\n";
        ...
        if (instance == NULL) return "Error occured in RNA class.\n";
        else return instance->GetRNA1()->GetErrorMessage(error);

    }

    else if (error < 5000) {
        if (instance == NULL) return "Error occured in Dynalign_object class.\n";
        else return instance->GetErrorMessage(error);
    }
...
int Multilign_object::ProgressiveMultilign(
  ...
  if(0!=(ErrorCode=PrepMultifindInput())) return ErrorCode;
    ...
    for (int j = 0; j < iterations; ++j){
...
#ifndef MULTIFIND      
            instance = new Dynalign_object(inputList[seqPair[i].first][0].c_str(), FILE_SEQ,
		...
		instance->GetRNA1()->SetTemperature(thermo.GetTemperature());
            ...
            if(!inputList[seqPair[i].first][2].empty()){
                ...
                if(0!=(ErrorCode=instance->GetRNA1()->ReadConstraints(inputList[seqPair[i].first][2].c_str()))) return ErrorCode;
            }
            ...
            if(!inputList[seqPair[i].second][2].empty()){
                ...
                if(0!=(ErrorCode=instance->GetRNA2()->ReadConstraints(inputList[seqPair[i].second][2].c_str()))) return ErrorCode;
            }
        ...
        if (!(i==0 && j==0) ){
                ...
                if(0!=(ErrorCode=instance->Templatefromdsv(tmpfilename.c_str(), maxDsv))) return ErrorCode;
            }
            ...
            instance->GetRNA1()->SetProgress(pp); // causes Dynalign to use the partial-progress object (as a proxy for this->progress)
	    ...
	    //	    cerr<<"i "<<i<<" j "<<j<<"\n";
			...
			if (0!=(ErrorCode = instance->Dynalign(maxtrace, bpwin, awin,
            ...
            if (j == iterations-1){
                ...
                if (i==seqPair.size()-1){
                    ...
                    if(0!=(ErrorCode=instance->GetRNA1()->WriteCt( inputList[seqPair[i].first][1].c_str() ))) {
            		      return ErrorCode;
            		    }
#else
		    if(ct_files.size())if(0!=(ErrorCode=instance->GetRNA1()->WriteCt( ct_files[seqPair[i].first].c_str() ))) {
                ...
                //else {
                    ...
                    //for(int n = 1; n <= struct_num; ++n) {
                        //rna->CalculateFreeEnergy(n);
                    //}
                    //if(0!=(ErrorCode=instance->GetRNA1()->WriteCt( inputList[seqPair[i].first][1] + '1').c_str() ))) return ErrorCode;
                //}
                ///// This is for debug/test./////////
...
#ifndef MULTIFIND	      
                if(0!=(ErrorCode=instance->GetRNA2()->WriteCt( inputList[seqPair[i].second][1].c_str() ))) return ErrorCode;
#else 
		//	cerr<<ct_files.size()<<"\n";
		if(ct_files.size())
		if(0!=(ErrorCode=instance->GetRNA2()->WriteCt( ct_files[seqPair[i].second].c_str() ) )) return ErrorCode;
#endif	       
            }
