...
int structure::ctout(const char * const ctoutfile, const bool append, const CTCommentProvider &commentProvider) const {
    int count,i;//length
    char line[2*ctheaderlength];//base[2]
    ofstream ctfile(ctoutfile,append ? std::ios_base::app : std::ios_base::trunc);
    if (!ctfile.good()) return 2; //cannot open file
    ...
    for (count=1;count<=(GetNumberofStructures());count++) {
        ...
        ctfile << line << endl; // write out header line
...
int structure::opendbn(const char *bracketFile) {
    ...
    ifstream infile(bracketFile);
    if (!(infile.good())) {
        SetErrorDetails(sfmt("Failed to open or read the file '%s'. Please check the file's permissions and verify that it is not empty.", bracketFile));
    ...
    // Read subsequent lines. Each is a new structure.
...
int structure::openct(const char *ctfile) {
    ...
    ifstream in(ctfile); // note: There is no need to close the ifstream before returning. It is closed in the ifstream destructor (which is called immediately when it goes out of scope).
    ...
    if (!in.read(base, 1).fail() && base[0]=='>') {
        // if the first character is '>' this is a dot-bracket file
        in.close(); // good idea to close our ifstream before calling opendbn. Ideally we could just pass our ifstream to an overload of opendbn
        return opendbn(ctfile); 
    } else 
        in.unget(); // put back the character that was read, It was probably just a space, but it could be a relevant part of the ct "count"
...
int structure::openseqx (const char *seqfile) {
    ...
    while ((reuseLine||!getline(infile,line).fail())&&!foundEndChar) {
        if (reuseLine) 
            reuseLine = false;
        else
            ++linenumber;
    ...
    if (fileType == SEQ && !foundEndChar) { 
        ...
        // report this error via the SetErrorDetails mechanism
        SetErrorDetails("The file was missing the required '1' that indicates the end of a sequence in a SEQ file.");
        return 29; //Invalid file format
    }
...
int structure::writedotbracket(const char * const filename, const int structurenumber, 
                               ...
                               const CTCommentProvider &commentProvider) const {
    ofstream out(filename);
    if (!out.good()) return 2; // could not open file.
    ...
    if (structurenumber!=-1) start=end=structurenumber;
    string label;
...
int structure::ReadExperimentalPairBonus(const char *filename, double const experimentalOffset, double const experimentalScaling ) {
    ...
    */
    ...
    int i( 1 ), j( 1 ), count( 0 ), valid_count( 0 );
    double val;
    ...
    //std::clog << "ExperimentalPairBonus input " << count << " values in total, " << valid_count << " values are valid" << std::endl;
...
// This is the default implementation of a functor to return a comment/label like "ENERGY = ..."
// that is inserted before the existing structure label when writing CT files 
// or dot-bracket files etc.
// A return value of NULL means that no label should be inserted.
// 
// The behavior of this default implementation is to return NULL if GetEnergy() is zero 
// (to match the behavior of legacy code).
string CTCommentProvider::getComment(const structure*const ct, const int structurenumber) const {
...
const CTCommentProvider CTCommentProvider::Default; // create the static Default CTCommentProvider
// The next two lines create the static NoComments CTCommentProvider. It is an instance of a derived class, a pointer/reference is used.
const auto_delete<CTCommentProvider> CTCommentProvider::noComments(CTCommentProvider::new_from(CTCommentProvider::noCommentsFunc)); 
const CTCommentProvider &CTCommentProvider::NoComments = *CTCommentProvider::noComments;
            ...
            if (element[k]<(alphabetSize+1)) {
                ...
                if (j>=2){
                    if (element[k-1]>(alphabetSize+1)) 
                        A[i][j]=min(A[i][j],A[i][j-2]+data->dangle[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][1]);
                }
                //Add terminal mismatch
                if (j>=3) {
                    if (element[k-2]<(alphabetSize+1) && element[k-1]>(alphabetSize+1))
                        A[i][j]=min(A[i][j],A[i][j-3]+data->tstkm[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]);
                }
                ...
                if (j>=4) {
                    if (element[k-3]>(alphabetSize+1) && element[k-1]>(alphabetSize+1) && element[k-2]<(alphabetSize+1))
                        A[i][j]=min(A[i][j],A[i][j-4]+data->coaxstack[decon2(element[k-3],alphabetSize)][decon1(element[k-3],alphabetSize)][element[k-2]][element[k]]+data->tstackcoax[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]);
                }
            }
                ...
                if (j>=2) {
                    if (element[k-1]<(alphabetSize+1))
                        A[i][j]=min(A[i][j],A[i][j-2]+data->dangle[decon2(element[k],alphabetSize)][decon1(element[k],alphabetSize)][element[k-1]][2]);
                }
                //Add flush coaxial
                if (j>=2) {
                    if (element[k-1]>(alphabetSize+1))
                        A[i][j]=min(A[i][j],A[i][j-2]+data->coax[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][decon1(element[k],alphabetSize)][decon2(element[k],alphabetSize)]);
                }
                //Add mismatch-mediated coaxial stacking
                if (j>=4) {
                    if (element[k-2]>(alphabetSize+1) && element[k-3]<(alphabetSize+1) && element[k-1]<(alphabetSize+1))
                        A[i][j]=min(A[i][j],A[i][j-4]+data->coaxstack[decon2(element[k],alphabetSize)][decon1(element[k],alphabetSize)][element[k-3]][element[k-1]]+data->tstackcoax[decon2(element[k-2],alphabetSize)][decon1(element[k-2],alphabetSize)][element[k-1]][element[k-3]]);
                }
            }
        ...
        if (element[k]<(alphabetSize+1)) {
            ...
            if (j>=2){
                if (element[k-1]>(alphabetSize+1) && ntraced) {
                    if (next_trace == A[i][j-2]+data->dangle[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][1]) {
                        next_trace=A[i][j-2];
                        ntraced=false;
                        j=j-2;
                    }
            ...
            if (j>=3) {
                if (element[k-2]<(alphabetSize+1) && element[k-1]>(alphabetSize+1) && ntraced) {
                    if (next_trace==A[i][j-3]+data->tstkm[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]) {
                        next_trace=A[i][j-3];
                        ntraced=false;
                        j=j-3;
                    }
            ...
            if (j>=4) {
                if (element[k-3]>(alphabetSize+1) && element[k-1]>(alphabetSize+1) && element[k-2]<(alphabetSize+1) && ntraced) {
                    if (next_trace==A[i][j-4]+data->coaxstack[decon2(element[k-3],alphabetSize)][decon1(element[k-3],alphabetSize)][element[k-2]][element[k]]+data->tstackcoax[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]) {
                        next_trace=A[i][j-4];
                        ntraced=false;
                        j=j-4;
                    }
            ...
            if (j>=2) {
                if (element[k-1]<(alphabetSize+1) && ntraced) {
                    if (next_trace==A[i][j-2]+data->dangle[decon2(element[k],alphabetSize)][decon1(element[k],alphabetSize)][element[k-1]][2]) {
                        next_trace=A[i][j-2];
                        ntraced=false;
                        j=j-2;
                    }
            ...
            if (j>=2) {
                if (element[k-1]>(alphabetSize+1) && ntraced) {
                    if (next_trace==A[i][j-2]+data->coax[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][decon1(element[k],alphabetSize)][decon2(element[k],alphabetSize)]) {
                        next_trace=A[i][j-2];
                        ntraced=false;
                        j=j-2;
                    }
            ...
            if (j>=4) {
                if (element[k-2]>(alphabetSize+1) && element[k-3]<(alphabetSize+1) && element[k-1]<(alphabetSize+1) && ntraced) {
                    if (next_trace==A[i][j-4]+data->coaxstack[decon2(element[k],alphabetSize)][decon1(element[k],alphabetSize)][element[k-3]][element[k-1]]+data->tstackcoax[decon2(element[k-2],alphabetSize)][decon1(element[k-2],alphabetSize)][element[k-1]][element[k-3]]) {
                        next_trace=A[i][j-4];
                        ntraced=false;
                        j=j-4;
                    }
        ...
        if (element[k]<(alphabetSize+1)) {//DALIA: remember the paired bases were multiplied by alphabetSize+1
            ...
            if (j>=2){
                if (element[k-1]>(alphabetSize+1)) //Check for dangle requirements
                    A[j]=min(A[j],A[j-2]+data->dangle[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][1]);
            }
            //Add terminal mismatch
            if (j>=3) {
                if (element[k-2]<(alphabetSize+1) && element[k-1]>(alphabetSize+1)) // Check for mismatch requirements
                    A[j]=min(A[j],A[j-3]+data->tstkm[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]);
            }
            ...
            if (j>=4) {
                if (element[k-3]>(alphabetSize+1) && element[k-1]>(alphabetSize+1) && element[k-2]<(alphabetSize+1)) // Check for mismatch-mediated coaxial stacking requirements
                    A[j]=min(A[j],A[j-4]+data->coaxstack[decon2(element[k-3],alphabetSize)][decon1(element[k-3],alphabetSize)][element[k-2]][element[k]]+data->tstackcoax[decon2(element[k-1],alphabetSize)][decon1(element[k-1],alphabetSize)][element[k]][element[k-2]]);
            }
        }
        else { //if the element is in a base pair
            //Add 5' Dangle
            if (j>=2) { 
                if (element[k-1]<(alphabetSize+1)) // Check for dangle requirements
                    A[j]=min(A[j],A[j-2]+data->dangle[decon2(element[k],alphabetSize)][decon1(element[k],alphabetSize)][element[k-1]][2]);
            }
            //Add flush coaxial
            if (j>=2) { 
                if (element[k-1]>(alphabetSize+1)) //Check for flush coaxial stacking requirements
                    A[j]=min(A[j],A[j-2]+data->coax[decon1(element[k-1],alphabetSize)][decon2(element[k-1],alphabetSize)][decon1(element[k],alphabetSize)][decon2(element[k],alphabetSize)]);
            ...
            if (j>=4) {
                if (element[k-2]>(alphabetSize+1) && element[k-3]<(alphabetSize+1) && element[k-1]<(alphabetSize+1)) // Check for mismatch-mediated coaxial stacking requirements
                    A[j]=min(A[j],A[j-4]+data->coaxstack[element[k-1]][element[k-3]][decon1(element[k],alphabetSize)][decon2(element[k],alphabetSize)]+data->tstackcoax[decon2(element[k-2],alphabetSize)][decon1(element[k-2],alphabetSize)][element[k-1]][element[k-3]]);
                    //cout << "mismatch_coax = " << min(A[j],A[j-4]+data->coaxstack[element[k-1]][element[k-3]][decon1(element[k],alphabetSize)][decon2(element[k],alphabetSize)]+data->tstackcoax[decon2(element[k-2],alphabetSize)][decon1(element[k-2],alphabetSize)][element[k-1]][element[k-3]]) << "   " << flush;
//                  cout << "coax 2 1 x x: " << decon2(element[k],alphabetSize) << decon1(element[k],alphabetSize) << element[k-3] << element[k-1]<< " tstack: 2 1 x x:  " << decon2(element[k-2],alphabetSize) << decon1(element[k-2],alphabetSize) << element[k-1] << element[k-3] << "  " << flush;
            }
        }
