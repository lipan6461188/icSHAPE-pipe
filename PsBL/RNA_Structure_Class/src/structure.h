...
class structure //this structure contains all the info for a structure
{
	...
	public:
		...
		//!         To provide a custom comment, pass an in an object drived from CTCommentProvider that has overridden 
		...
		//!     To disable comments, pass in CTCommentProvider::NoComments. 
		//!     The default is CTCommentProvider::Default, which provides legacy "ENERGY = ..." comments.
		int ctout(const char * const ctoutfile, const bool append=false, const CTCommentProvider &commentProvider=DEFAULT_CT_ENERGY_COMMENTS) const;		
		...
		//! \param singleHeader If multiple structures are being written (i.e. structurenumber==-1), this determines whether 
		//!        the structure title and sequence are written once at the top followed by multiple lines of brackets (singleHeader = true)
		//!        or the structure title and sequence are written out individually for each structure (singleHeader = false).
		...
		int writedotbracket(const char * const filename, const int structurenumber=-1, 
			...
			const CTCommentProvider &commentProvider=DEFAULT_CT_ENERGY_COMMENTS) const;
