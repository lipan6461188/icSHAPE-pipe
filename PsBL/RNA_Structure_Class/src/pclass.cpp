...
inline void Pclass::fill() {
	...
	rarray= (PFPRECISION) 0;//initiated the rarray to be zero as energy would be infinity
...
sub2:
	  ...
	  rarray = (PFPRECISION) 0;//reuse rarray to store the value
...
OligoPclass::OligoPclass(structure *CT, pfdatatable *DATA):Pclass(CT,DATA) {
		...
		copyw5[0] = (PFPRECISION) 1;
		for (i=0;i<=number;i++) {
			copywca[i]  = new PFPRECISION [number+1];
			for (j=0;j<=number-i;j++) {
				copywca[i][j]= (PFPRECISION) 0;
			}
		}
}
